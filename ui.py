...
class Gui:
    def __init__(self):
        ...
        self.board_img = pygame.image.load(os.path.join(
            "imgs", "board.bmp")).convert()
        self.black_img = pygame.image.load(os.path.join(
            "imgs", "black.bmp")).convert()
        self.white_img = pygame.image.load(os.path.join(
            "imgs", "white.bmp")).convert()
        self.tip_img = pygame.image.load(os.path.join("imgs",
                                                      "tip.bmp")).convert()
        self.clear_img = pygame.image.load(os.path.join("imgs",
                                                        "em.bmp")).convert()
    ...
    def show_options(self):
        ...
        while True:
            ...
            title = title_fnt.render("Othello Board Game", True, self.WHITE)
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=60)

            start_txt = self.font.render("Let's begin", True, self.WHITE)
            start_pos = start_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=220)
            player1_txt = self.font.render("Please select First Player", True, self.WHITE)
            player1_pos = player1_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=260)
            player2_txt = self.font.render("Please select second Player", True, self.WHITE)
            player2_pos = player2_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=300)
            level_txt = self.font.render("Please Select Computer Intelligence", True, self.WHITE)
    ...
    def show_winner(self, player_color):
        ...
        while True:
            ...
            if player_color == WHITE:
                ...
                msg = font.render("It's a Tie !", True, self.WHITE)
            self.screen.blit(
                msg, msg.get_rect(
    ...
    def get_chosen_level(self):
        ...
        while True:
            ...
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=60)
            one_txt = self.font.render("Beginner", True, self.WHITE)
            one_pos = one_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=160)
            two_txt = self.font.render("Intermediate", True, self.WHITE)
            two_pos = two_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=240)

            three_txt = self.font.render("Expert", True, self.WHITE)
